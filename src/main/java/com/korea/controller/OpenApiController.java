...
public class OpenApiController {
		...
		String OpenApiInsert(CompanyVO vo, Model model) throws Exception{
		  StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/6480000/gyeongnamgoodemploycompany/gyeongnamgoodemploycompanylist"); /*URL*/
	        urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=p%2BZTpvUVwzVNEou0UmhhMF%2B15HUA7y5VL2SMgEBTFAgaDKzENXi9EINbIdVAO%2FM%2F%2BzW1xHc5VbG1NWV7hyPkUA%3D%3D"); /*Service Key (일반인증키)*/
	        ...
	        try {
	         ...
	         //service.deleteAll();  //기존값 지우고 insert하기 위해서
		...
		String EvInsert(EvSearchVO vo, Model model) throws Exception{
		  StringBuilder urlBuilder = new StringBuilder("http://openapi.kepco.co.kr/service/EvInfoServiceV2/getEvSearchList"); /*URL*/
	        ...
	        try {
	         ...
	         data2 = item.getElementsByTagName("cpNm").item(0); // 이름
	         data3 = item.getElementsByTagName("cpStat").item(0); // 상태
	         data4 = item.getElementsByTagName("csNm").item(0); // 상태
		...
		String company(CompanyVO vo, Model model) throws Exception{
			 model.addAttribute("li", service.list(vo));
			return "/openApi/Company.jsp";
		}
		@RequestMapping("/Map.do")
		String Map(CompanyVO vo, Model model) throws Exception{
			 model.addAttribute("li", service.list(vo));
			return "/openApi/Map.jsp";
		}
		
		@RequestMapping("/Map_Edit.do")
		String Map_Edit(CompanyVO vo, Model model) throws Exception{
			 model.addAttribute("m", service.edit(vo));
			return "/openApi/Map_Edit.jsp";
		}
		
		
		@RequestMapping("/EvSearch.do")
		String EvSearch(EvSearchVO vo, Model model) throws Exception{
			 model.addAttribute("li", service.listEv(vo));
			return "/openApi/EvSearch.jsp";
		}
		
		@RequestMapping("/EvSearchMap.do")
		String EvSearchMap(EvSearchVO vo, Model model) throws Exception{
			 model.addAttribute("li", service.listEv(vo));
			return "/openApi/EvSearchMap.jsp";
		}
		@RequestMapping("/EvSearchOne.do")
		String EvSearchOne(EvSearchVO vo, Model model) throws Exception{
			 model.addAttribute("m", service.listEvOne(vo));
			return "/openApi/EvSearchOne.jsp";
		}
}
